...
export default function DeletePlanDrawer({
    isOpen,
    planName,
    onClose,
    onConfirm,
    ...
    const isMatch = confirmName.trim().toLowerCase() === planName.trim().toLowerCase();
                ...
                >
                    ...
                    <div className="p-6 border-b border-light-border flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-sm font-semibold text-black tracking-tight">Delete Subscription Plan</h2>
                                <p className="text-xs text-black/40 font-normal">Permanently remove subscription tier from SaaS</p>
                            </div>
                        </div>
                        <button
                            type="button"
                    ...
                    <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6 slim-scrollbar text-xs">
                            ...
                            <div className="flex items-start gap-2.5">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs mt-1.5 leading-relaxed text-black/70 font-medium">
                                        You are about to permanently delete the subscription tier <strong className="font-mono bg-red-100 px-1 py-0.5 rounded text-red-700 text-xs font-bold">{planName}</strong>.
                                    </p>
                                </div>
                            </div>

                            <ul className="text-xs text-black/70 font-medium space-y-2 list-disc pl-4 pt-1">
                                <li>All linked pricing packages (Monthly, Half-Yearly, Annually) will be destroyed.</li>
                                <li>No new schools will be able to subscribe to this plan tier.</li>
                                <li>This operation cannot be performed if active client schools are subscribed.</li>
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-xs text-black/45 leading-normal">
                                To proceed with the deletion, please type the plan name <strong className="font-semibold text-black">{planName}</strong> below:
                            </p>
                            <input
                                type="text"
                                required
                                placeholder={`e.g. ${planName}`}
                                value={confirmName}
                                onChange={(e) => setConfirmName(e.target.value)}
                                className="w-full border border-red-200 focus:border-red-600 outline-none p-2.5 rounded-lg text-xs font-mono font-bold text-red-600 placeholder-black/25 transition bg-white"
                            />
                        </div>
                    </form>
