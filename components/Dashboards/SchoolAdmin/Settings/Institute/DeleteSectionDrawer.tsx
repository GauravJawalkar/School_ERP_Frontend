...
interface DeleteSectionDrawerProps {
    isOpen: boolean;
    sectionName: string;
    className: string;
    onClose: () => void;
...
export default function DeleteSectionDrawer({
    isOpen,
    sectionName,
    className,
    onClose,
    ...
    const isMatch = confirmName.trim().toUpperCase() === sectionName.trim().toUpperCase();
                ...
                >
                    ...
                    <div className="p-6 border-b border-light-border flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-sm font-semibold text-black tracking-tight">Delete Class Section</h2>
                                <p className="text-xs text-black/40 font-normal">Permanently remove section partition.</p>
                            </div>
                        </div>
                        <button
                            type="button"
                    ...
                    <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6 slim-scrollbar text-xs">
                            ...
                            <div className="flex items-start gap-2.5">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs mt-1.5 leading-relaxed text-black/70 font-medium">
                                        You are about to permanently delete <strong className="font-mono bg-red-100 px-1 py-0.5 rounded text-red-700 text-xs font-bold">Section {sectionName}</strong> from <strong className="text-black">Class {className}</strong>.
                                    </p>
                                </div>
                            </div>

                            <ul className="text-xs text-black/70 font-medium space-y-2 list-disc pl-4 pt-1">
                                <li>All student enrollments mapped to this section partition will be orphaned.</li>
                                <li>Timetable structures and teacher allocations for this section will be removed.</li>
                        ...
                        <div className="space-y-2">
                            <label className="text-xs font-bold text-black/70 uppercase tracking-wider block">
                                Type Section Name to Confirm
                            </label>
                            <p className="text-xs text-black/45 leading-normal">
                                To proceed with the deletion, please type the section name <strong className="font-semibold text-black">{sectionName}</strong> below:
                            </p>
                            <input
                                type="text"
                                required
