...
import { Role } from "../types/rolesPermissions.types";
...
export default function RoleCard({ role, isSelected, onSelect }: RoleCardProps) {
    ...
    return (
        <button
            type="button"
            onClick={onSelect}
            className={`w-full text-left p-5 rounded-xl border transition-all duration-300 relative overflow-hidden group cursor-pointer flex flex-col justify-between min-h-42.5 ${isSelected
        ...
        >
            <div className="space-y-3 w-full">
                <div className="flex items-center justify-between w-full">
                    ...
                    <span
                        className={`text-[9px] font-bold px-2.5 py-0.5 rounded-full uppercase tracking-wider ${isSelected
            ...
            <div className={`border-t mt-4 pt-3 flex items-center justify-between text-xs font-semibold w-full ${isSelected ? "border-white/10" : "border-light-border"}`}>
                ...
                <span
                    className={`text-xs font-bold transition flex items-center gap-1 ${isSelected ? "text-white" : "text-black/0 group-hover:text-black/60"
                        }`}
                >
                    <Eye size={12} />
                    Inspect Matrix
