...
export default function SchoolUsersTable({
    ...
    const availableRoles = useMemo(() => {
        const unique = Array.from(new Set(users.map((u) => u.roleName).filter(Boolean))) as string[];
        return [
            { label: "All Roles", value: "ALL" },
            ...unique.map((role) => ({
                label: role.replace(/_/g, " "),
                value: role,
                countFilter: (u: SystemUser) => u.roleName === role
            }))
        ];
    }, [users]);
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2.5 text-[10px] text-black/40 font-medium mt-0.5">
                                ...
                                {row.employeeCode && <span>• Code: {row.employeeCode}</span>}
                            </div>
                        </div>
                    </div>
                )
