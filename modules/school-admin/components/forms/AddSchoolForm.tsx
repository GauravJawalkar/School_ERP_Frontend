...
import TagChipInput from "@/shared_components/Forms/TagChipInput"
import StepIndicator from "@/shared_components/Forms/StepIndicator"
...
const AddSchoolForm = ({ mode = 'edit', schoolSlug, defaultData, initialData, isEditMode, onSuccess }: AddSchoolFormProps) => {
    const [step, setStep] = useState(0)
    const effectiveData = initialData || defaultData;
    const effectiveMode = isEditMode !== undefined ? (isEditMode ? 'edit' : 'create') : mode;

    const form = useForm({
        defaultValues: {
            schoolName: effectiveData?.schoolName || '',
            primaryEmail: effectiveData?.primaryEmail || '',
            affiliationNumber: effectiveData?.affiliationNumber || '',
            instituteLogo: undefined as unknown as File,
            medium: effectiveData?.medium || '',
            establishedYear: effectiveData?.establishedYear || '',
            main_phone: effectiveData?.main_phone || '',
            website: effectiveData?.website || '',
            city: effectiveData?.city || '',
            state: effectiveData?.state || '',
            address: effectiveData?.address || '',
            landmark: effectiveData?.landmark || '',
            office_hours_Mon_Fri: effectiveData?.office_hours_Mon_Fri || '',
            office_hours_Sat: effectiveData?.office_hours_Sat || '',
            pincode: effectiveData?.pincode || '',
            founderName: effectiveData?.founderName || '',
            missionStatement: effectiveData?.missionStatement || '',
            visionStatement: effectiveData?.visionStatement || '',
            coreValues: effectiveData?.coreValues || [] as string[],
            tags: effectiveData?.tags || [] as string[],
            boardsAffiliated: effectiveData?.boardsAffiliated || [] as string[],
            notableAlumni: effectiveData?.notableAlumni || [] as string[],
        ...
        <section className="p-6 rounded-md shadow-xs border border-light-border">
            <div className="mb-8">
                <h2 className="text-base font-medium">
                    {effectiveMode === 'edit' ? 'Edit School Details' : 'Add New School'}
                </h2>
                ...
                {step === 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <form.Field
                            name="instituteLogo"
                            validators={{
                                onChange: effectiveMode === 'edit' ? undefined : schoolSchema.shape.instituteLogo,
                                onBlur: effectiveMode === 'edit' ? undefined : schoolSchema.shape.instituteLogo,
                            }}
                            children={(field) => {
                                ...
                                const previewUrl = selectedFile instanceof File
                                    ...
                                    : (effectiveMode === 'edit' && effectiveData?.instituteLogo ? (effectiveData.instituteLogo as unknown as string) : "");
                                ...
                                return (
                                    <div className="flex flex-col gap-1.5">
                                        <label className="text-sm font-medium text-black">
                                            Institute Logo {effectiveMode === 'create' && <RequiredBadge />}
                                        </label>
                    ...
                    {step < STEPS.length - 1 ? (
                        ...
                        <button
                            ...
                            {addNewSchoolMutation.isPending
                                ? <><Loader2 size={15} className="animate-spin" /> {effectiveMode === 'edit' ? 'Saving...' : 'Adding...'}</>
                                : <><CheckIcon size={15} /> {effectiveMode === 'edit' ? 'Save Changes' : 'Add School'}</>
                            }
                        </button>
                    )}
                </div>
