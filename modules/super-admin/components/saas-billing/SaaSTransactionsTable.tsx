                ...
                <table className="w-full text-xs min-w-max">
                    ...
                    <tbody className="divide-y divide-light-border bg-white text-black/70">
                        {filtered.length === 0 ? (
                            ...
                            filtered.map((tx) => {
                                const formattedDate = tx.invoiceDate ? new Date(tx.invoiceDate).toLocaleDateString("en-IN", {
                                    day: "2-digit",
                                    month: "short",
                                    year: "numeric"
                                }) : "N/A";
                                const formattedDueDate = tx.dueDate ? new Date(tx.dueDate).toLocaleDateString("en-IN", {
                                    day: "2-digit",
                                    month: "short",
                                    year: "numeric"
                                }) : "N/A";
                                    ...
                                    <tr key={tx.invoiceId} className="hover:bg-gray-50/40 transition">
                                        ...
                                        <td className="px-4 py-3.5">
                                            <div>
                                                ...
                                                <span className="text-xs text-black/40 font-medium flex items-center gap-1 mt-0.5 lowercase font-mono">
                                                    ...
                                                    {tx.schoolSlug || "school"}.layernlooms.com
                                                </span>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-4 py-3.5">
                                            <span className="font-semibold text-black/60 capitalize font-mono text-[10px] border border-light-border bg-gray-50/50 px-2 py-0.5 rounded-sm">
                                                {(tx.paymentMethod || "MANUAL").replace("_", " ").toLowerCase()}
                                            </span>
                                        </td>
