...
import SaaSSubscriptionDistribution from "../components/saas-subscriptions/SaaSSubscriptionDistribution";
import SaaSActiveContractsTable from "../components/saas-subscriptions/SaaSActiveContractsTable";
import SaaSPlanDrawer from "../components/saas-subscriptions/SaaSPlanDrawer";
import SaaSCreatePlanDrawer from "../components/saas-subscriptions/SaaSCreatePlanDrawer";
import DeletePlanDrawer from "../components/saas-subscriptions/DeletePlanDrawer";
...
export function SaaSSubscriptionsPage() {
    ...
    const getAllContracts = async (): Promise<Contract[]> => {
        ...
        const list = Array.isArray(rawData) ? rawData : rawData?.subscriptions ?? [];

        return list.map((item: any) => ({
            contractId: item.contractId || String(item.id),
            instituteId: item.instituteId,
            schoolName: item.schoolName || item.instituteName || `School #${item.instituteId}`,
            schoolSlug: item.schoolSlug || "school",
            tierName: item.tierName || item.planName || "Standard Plan",
            billingCycle: item.billingCycle || item.billingPeriod || "MONTHLY",
            price: Number(item.price || item.amount || 0),
            billingStatus: item.billingStatus || item.status || "ACTIVE",
            startDate: item.startDate || new Date().toISOString(),
            renewalDate: item.renewalDate || item.endDate || new Date().toISOString(),
            lastPaymentTxId: item.lastPaymentTxId
        }));
    };
    ...
    const getSaaSPlans = async (): Promise<SaaSPlan[]> => {
        ...
        const rawData = response.data?.data ?? response.data;
        const backendPlans = Array.isArray(rawData) ? rawData : rawData?.plans ?? [];

        return backendPlans.map((p: any) => {
            return {
                planId: p.slug || p.name?.toLowerCase().replace(/\s+/g, "-") || `plan-${p.id}`,
                name: p.name,
                studentLimit: p.maxStudents ?? p.studentLimit ?? 500,
                staffLimit: p.maxStaff ?? p.staffLimit ?? 50,
                features: Array.isArray(p.features) ? p.features : p.features?.modules || [],
                isActive: p.isActive ?? true,
                dbPlanId: p.id,
                prices: p.prices || [
                    { id: p.id, billingPeriod: "MONTHLY", amount: String(p.price || 0) },
                    { id: p.id, billingPeriod: "HALF_YEARLY", amount: String((p.price || 0) * 5) },
                    { id: p.id, billingPeriod: "ANNUALLY", amount: String((p.price || 0) * 10) }
                ]
            };
        });
    };
    ...
    const createPlanMutation = useMutation({
        mutationFn: async (payload: {
            ...
            const newPlanId = planRes.data?.data?.id || planRes.data?.id;
                ...
                {activeTab === "plans" && (
                    <div className="space-y-6 animate-in fade-in duration-200">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {plans
                                .filter((plan) => plan.prices?.some((pr) => pr.billingPeriod === billingCycleFilter) || true)
                                .map((plan) => {
                                    const matchedPrice = plan.prices?.find(pr => pr.billingPeriod === billingCycleFilter) || plan.prices?.[0];
