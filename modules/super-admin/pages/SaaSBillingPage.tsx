...
// Child imports
import SaaSBillingStats from "../components/saas-billing/SaaSBillingStats";
import SaaSBillingHistoryChart from "../components/saas-billing/SaaSBillingHistoryChart";
import SaaSTransactionsTable from "../components/saas-billing/SaaSTransactionsTable";
import SaaSInvoiceDrawer from "../components/saas-billing/SaaSInvoiceDrawer";
...
export function SaaSBillingPage() {
    ...
    const getAllTransactions = async (): Promise<Transaction[]> => {
        ...
        const list = Array.isArray(rawData) ? rawData : rawData?.transactions ?? [];

        return list.map((item: any) => ({
            invoiceId: item.invoiceId || item.id || `INV-${item.contractId || Date.now()}`,
            schoolName: item.schoolName || item.instituteName || `School #${item.instituteId || 1}`,
            schoolSlug: item.schoolSlug || "school",
            amount: Number(item.amount || item.price || 0),
            paymentMethod: item.paymentMethod || item.paymentGateway || "STRIPE",
            status: item.status || item.billingStatus || "UNPAID",
            invoiceDate: item.invoiceDate || item.transactionDate || item.createdAt || new Date().toISOString(),
            dueDate: item.dueDate || item.renewalDate || new Date().toISOString()
        }));
    };
